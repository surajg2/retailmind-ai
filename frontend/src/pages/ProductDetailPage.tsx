  ...
  return (
    <div className="page-data-wipe space-y-6 pb-12">
      ...
      <div>
        <button
          onClick={() => navigate('/products')}
          className="tactile-button inline-flex items-center gap-2 text-xs font-semibold text-cyan-400 hover:text-cyan-300 transition-colors mb-3"
        >
          <ArrowLeft className="w-4 h-4" /> Back to Products Catalog
        </button>

        <div className="tactile-card bg-slate-900/80 border border-slate-800 rounded-2xl p-6 shadow-xl backdrop-blur-md flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
          ...
          <div className="flex items-center gap-6 font-mono">
            <div>
              <span className="text-[10px] font-semibold text-slate-400 block font-sans">Selling Price</span>
              <span className="text-lg font-bold text-emerald-400">₹{Number(product.selling_price).toFixed(2)}</span>
            </div>
            <div>
              <span className="text-[10px] font-semibold text-slate-400 block font-sans">Cost Price</span>
              <span className="text-lg font-bold text-slate-300">₹{Number(product.cost_price).toFixed(2)}</span>
            </div>
            <div>
              <span className="text-[10px] font-semibold text-slate-400 block font-sans">Min Stock Threshold</span>
      ...
      <div className="flex items-center justify-between bg-slate-900/80 border border-slate-800 rounded-xl p-3 backdrop-blur-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="tactile-card bg-slate-900/80 rounded-2xl p-4 backdrop-blur-sm">
          ...
          <div className="text-2xl font-bold text-slate-100 mt-2 font-mono animate-inventory-pulse">{totalUnits.toLocaleString()} pcs</div>
          <span className="text-[10px] text-slate-500 mt-1 block">In selected timeframe</span>
        </div>

        {/* Confirmed Stockouts with Stockout Event Localized Ripple */}
        <div className="tactile-card bg-slate-900/80 rounded-2xl p-4 backdrop-blur-sm relative overflow-hidden">
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 shadow-xl backdrop-blur-md">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-slate-200 tracking-wide">Daily Observed Sales & Inventory Trajectory</h3>
          </div>
          ...
          {performance.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={performance} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                />
                <Bar yAxisId="left" dataKey="units_sold" name="Observed Units Sold" fill="#06b6d4" radius={[4, 4, 0, 0]} isAnimationActive={true} animationDuration={850} />
            ...
            <div className="empty-data-grid h-full rounded-xl border border-slate-800/80 flex items-center justify-center text-slate-500 text-xs">
