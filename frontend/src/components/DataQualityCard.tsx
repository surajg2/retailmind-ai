...
export const DataQualityCard: React.FC<DataQualityCardProps> = ({ report }) => {
  ...
  return (
    <div className="data-scan-line bg-zinc-900/80 border border-zinc-800 rounded-2xl p-5 shadow-xl backdrop-blur-md relative overflow-hidden">
      
      <div className="flex items-center justify-between mb-4 relative z-10">
        <div className="flex items-center gap-2">
          <ShieldCheck className="w-5 h-5 text-zinc-300" />
          <h3 className="text-sm font-bold text-zinc-200 tracking-wide">Data Integrity & Quality Score</h3>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 relative z-10">
        ...
        {/* Quality Score Bar */}
        <div className="bg-zinc-950/80 rounded-xl p-3.5 border border-zinc-800/80">
          <div className="text-[11px] font-medium text-zinc-400 mb-1 flex items-center justify-between">
            <span>Overall Score</span>
            <span className="font-bold text-zinc-200 font-mono">{report.quality_score}%</span>
          </div>
          <div className="w-full bg-zinc-800/80 rounded-full h-2 overflow-hidden">
            <div
              className="bg-gradient-to-r from-zinc-400 to-zinc-100 h-full rounded-full transition-all duration-700 ease-out"
              style={{ width: `${Math.min(100, Math.max(0, report.quality_score))}%` }}
            />
          </div>
          <span className="text-[10px] text-zinc-500 mt-1 block">Based on completeness & coverage</span>
        </div>

        {/* Date Coverage */}
        <div className="bg-zinc-950/80 rounded-xl p-3.5 border border-zinc-800/80">
          <div className="text-[11px] font-medium text-zinc-400 mb-1 flex items-center gap-1.5">
            <Calendar className="w-3.5 h-3.5 text-zinc-300" /> Date Coverage
          </div>
          <div className="text-lg font-bold text-zinc-100 font-mono">
            {report.total_recorded_days} <span className="text-xs font-normal text-zinc-400">/ {report.expected_days} days</span>
          </div>
        ...
        <div className="bg-zinc-950/80 rounded-xl p-3.5 border border-zinc-800/80">
          ...
          <div className="text-lg font-bold text-amber-400 font-mono">
            {report.date_gaps_count} <span className="text-xs font-normal text-zinc-400">gaps</span>
          </div>
          <span className="text-[10px] text-zinc-500 mt-0.5 block">Unrecorded calendar dates</span>
        </div>
        ...
        <div className="bg-zinc-950/80 rounded-xl p-3.5 border border-zinc-800/80">
          ...
          <div className="flex items-center gap-1 text-[10px] text-zinc-400 mt-0.5" title="Operational business indicator, not bad data">
